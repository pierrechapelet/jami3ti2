...
class UsersController extends AppController
{
    ...
    // // Used to reencode existing clear passwords...
    // public function recodePasswords()
    // {
    //     $this->viewBuilder()->layout('ajax');
    //     $users = $this->Users->find()->all();
    //     foreach ($users as $user)
    //     {
    //         if (strlen($user->clearpass) > 0) {
    //             $user['password'] = (new DefaultPasswordHasher)->hash($user->clearpass);
    //         }
    ...
    public function viewProvider($id = null)
    {
        ...
        $this->set('user', $user);
        $this->set('_serialize', ['user']);
        $ProviderMenu = $this->MenuBuilder->BuildProviderUserMenu('provider_menu_account', $id);
    ...
    public function registerApplicant()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if($this->request->data['agree_with_disclaimer'] == 1) {
                if ($this->Users->save($user)) {
                    $this->Flash->success(__('The record has been saved.'));
                ...
                $this->Flash->error(__('You need to agree with the disclaimer'));
            }
        }
        $this->set(compact('user'));
        $this->set('_serialize', ['user']);
