...
class CountriesController extends AppController
{
    ...
    public function view($code = null)
    {
        ...
        //find the number of available opportunities
        $countryId = $country->id;
        $nbOpportunities = TableRegistry::get('Opportunities')->find()
            ->contain(['OpportunityCountries'])
            ->select(['count' => $query->func()->sum('seats')])
            ->matching(
                'OpportunityCountries', function ($q) use ($countryId) {
                    return $q->where(['country_id' => $countryId]);
                })
            ->toArray();

        //debug($nbOpportunities[0]->count);
        $this->set('nb_opportunities', $nbOpportunities[0]->count);



        $latestOpportunities = TableRegistry::get('Opportunities')->find()
            ->contain(['OpportunityCountries'])
            ->matching(
                'OpportunityCountries', function ($q) use ($countryId) {
                    return $q->where(['country_id' => $countryId]);
                })
            ->order(['application_end_date' => 'DESC'])
            ->all();

        $this->set('latestOpportunities', $latestOpportunities);
